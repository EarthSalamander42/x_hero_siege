...
function Physics:PhysicsTestCommand(...)
	...
	if string.find(text, "^units") then
		...
		if m ~= nil then
			...
			for i = 1, m do
				local unit = CreateUnitByName(dummy_blank, hero:GetAbsOrigin(), true, hero, hero, hero:GetTeamNumber())
...
Physics:CreateColliderProfile("aaboxblocker", {
	...
	action = function(self, box, unit)
		...
		Physics:BlockInAABox(unit, xblock, value, self.buffer, self.findClearSpace)
