-- Imported from Dota IMBA commit c9ed3556eb64a2f950cbecb8f2fb09f1e352996f.
-- XHS compatibility: load only the four root KV files shipped by this addon.
...
--				print("[KeyValues] Warning: Duplicated unit/hero entry for "..key)
	...
	for key,value in pairs(KeyValues.HeroKV2 or {}) do
		if KeyValues.UnitKV[key] ~= key then
			KeyValues.UnitKV[key] = value
		else
			if type(KeyValues.All[key]) == "table" then
--				print("[KeyValues] Warning: Duplicated unit/hero entry for "..key)
	...
	for key,value in pairs(KeyValues.AbilityKV2 or {}) do
...
function GetKeyValueByHeroName(hero_name, key, level)
