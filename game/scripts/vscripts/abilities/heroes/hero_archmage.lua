...
function modifier_holdout_mana_shield:OnTooltip()
	return self.absorption_pct or 0
end

function modifier_holdout_mana_shield:OnTooltip2()
	return self.damage_per_mana or 0
end
...
function RainOfIce( event )
	...
	for i = 1, explosions_per_tick do
		...
		local moonstrike_inner = ParticleManager:CreateParticle("particles/econ/items/crystal_maiden/crystal_maiden_maiden_of_icewrack/maiden_freezing_field_cracks_arcana.vpcf", PATTACH_CUSTOMORIGIN, caster)
		ParticleManager:SetParticleControl(moonstrike_inner, 0, point)
		...
		Timers:CreateTimer(delay - 0.1, function()
			local impact = ParticleManager:CreateParticle("particles/econ/items/crystal_maiden/crystal_maiden_maiden_of_icewrack/maiden_freezing_field_explosion_arcana1.vpcf", PATTACH_CUSTOMORIGIN, caster)
			ParticleManager:SetParticleControl(impact, 0, point)
			caster:EmitSound("Hero_Invoker.SunStrike.Ignite")
		end)

		Timers:CreateTimer(delay, function()
			local impacted_units = FindUnitsInRadius(
				caster:GetTeamNumber(),
				point,
				nil,
				radius_explosion,
			...
			for _, unit in pairs(impacted_units) do
				ApplyDamage({
					victim = unit,
					attacker = caster,
					ability = ability,
