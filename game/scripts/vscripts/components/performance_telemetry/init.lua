	...
	for playerID = 0, maximumPlayers - 1 do
		...
		if PlayerResource:IsValidPlayerID(playerID)
			...
			local player = {
				...
				alive = hero:IsAlive(),
				level = hero:GetLevel(),
				kills = math.max(0, tonumber(PlayerResource:GetKills(playerID)) or 0),
				deaths = math.max(0, tonumber(PlayerResource:GetDeaths(playerID)) or 0),
				assists = math.max(0, tonumber(PlayerResource:GetAssists(playerID)) or 0),
