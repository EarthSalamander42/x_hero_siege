...
function GameMode:ChooseTreasureSurprise( hPlayerHero, hTreasureEnt )
	hTreasureEnt.nRewardSpawnDist = 64

	if hTreasureEnt.zone == nil then
		if GetMapName() == "ep_1" then
			print( "GameMode:ChooseTreasureSurprise() -- WARNING: No zone found for this chest. Defaulting to \"forest\"" )
			hTreasureEnt.zone = self:GetZoneByName( "forest" )
		elseif GetMapName() == "ep_2" then
			print( "GameMode:ChooseTreasureSurprise() -- WARNING: No zone found for this chest. Defaulting to \"tundra\"" )
			hTreasureEnt.zone = self:GetZoneByName( "tundra" )
		else
			print( "GameMode:ChooseTreasureSurprise() -- ERROR: No zone found for this chest and can't set a reasonable default because we're not in map \"ep_1\" or \"ep_2\"!" )
			return
		end
	end
	...
	if hTreasureEnt.Items == nil then
		print( "GameMode:ChooseTreasureSurprise() -- WARNING: No items table found for this chest. Will give you some branches instead." )
		hTreasureEnt.Items =
		{
			"item_branches",
		}
	...
	if hTreasureEnt.nMinGold == nil then
		print( "GameMode:ChooseTreasureSurprise() -- WARNING: Missing nMinGold value for this chest. Will use a default value based on player level instead." )
		hTreasureEnt.nMinGold = hPlayerHero:GetLevel() * 150
	end

	if hTreasureEnt.nMaxGold == nil then
		print( "GameMode:ChooseTreasureSurprise() -- WARNING: Missing nMaxGold value for this chest. Will use a default value based on player level instead." )
	...
	if bPreventTraps == false and (hTreasureEnt.szTraps == nil or #hTreasureEnt.szTraps == 0) then
		print( "GameMode:ChooseTreasureSurprise() -- WARNING: Field named szTraps is missing or empty for this chest.  Using default data instead." )
	...
	if hTreasureEnt.nTrapLevel == nil or hTreasureEnt.nTrapLevel <= 0 then
		print( "GameMode:ChooseTreasureSurprise() -- WARNING: Field named nTrapLevel is missing or is 0 or less.  Using default data instead." )
		hTreasureEnt.nTrapLevel = 1
	end

	if hTreasureEnt.fRelicChance == nil then
		hTreasureEnt.fRelicChance = 0.01
		--print( string.format( "GameMode:ChooseTreasureSurprise() -- WARNING: No fRelicChance specified for this chest. Setting it to %.2f", hTreasureEnt.fRelicChance ) )
	end

	if hTreasureEnt.fItemChance == nil then
		hTreasureEnt.fItemChance = 0.3
		--print( string.format( "GameMode:ChooseTreasureSurprise() -- WARNING: No fItemChance specified for this chest. Setting it to %.2f", hTreasureEnt.fItemChance ) )
	end
...
function GameMode:CreateTreasureItemDrop( hPlayerHero, hTreasureEnt )
...
function GameMode:CreateTreasureRelicDrop( hPlayerHero, hTreasureEnt )
...
function GameMode:CreateTreasureGoldDrop( hPlayerHero, hTreasureEnt )
...
function GameMode:ChooseTreasureTrap( hPlayerHero, hTreasureEnt )
...
function GameMode:CreateTreasureMineTrap( hPlayerHero, hTreasureEnt )
	...
	if hLandMineAbility == nil then
		print( string.format( "GameMode:OnTreasureOpen - ERROR: %s is missing required ability", hTreasureEnt:GetUnitName() ) )
		return
	end
...
function GameMode:CreateTreasureSunStrikeTrap( hPlayerHero, hTreasureEnt )
...
function GameMode:GetChestRewardSpawnPos( hTreasureEnt )
	local vPos = hTreasureEnt:GetAbsOrigin() + RandomVector( hTreasureEnt.nRewardSpawnDist )
