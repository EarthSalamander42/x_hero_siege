...
function Spawn( entityKeyValues )
    ...
    behaviorSystem = AICore:CreateBehaviorSystem( { BehaviorNone, BehaviorSpikedCarapace, BehaviorRunAway } ) 
end
...
BehaviorSpikedCarapace = {}

function BehaviorSpikedCarapace:Evaluate()
	local desire = 0

	-- let's not choose this twice in a row
	if currentBehavior == self then return desire end

	ABILITY_SpikedCarapace = thisEntity:FindAbilityByName( "creature_spiked_carapace" )
	...
	if #enemies > 0 then
		desire = 4
		if ABILITY_SpikedCarapace and ABILITY_SpikedCarapace:IsFullyCastable() then
			thisEntity:Stop()
			thisEntity:CastAbilityNoTarget( ABILITY_SpikedCarapace, -1 )
		end
	end
...
function BehaviorSpikedCarapace:Begin()
	self.endTime = GameRules:GetGameTime() + 1
	...
	self.order =
	{
		...
		AbilityIndex = ABILITY_SpikedCarapace:entindex(),
	}

end

BehaviorSpikedCarapace.Continue = BehaviorSpikedCarapace.Begin
...
AICore.possibleBehaviors = { BehaviorNone, BehaviorSpikedCarapace, BehaviorRunAway }
