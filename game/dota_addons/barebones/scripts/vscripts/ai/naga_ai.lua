...
function Spawn( entityKeyValues )
    ...
    behaviorSystem = AICore:CreateBehaviorSystem( { BehaviorNone, BehaviorStarfall, BehaviorRunAway } ) 
end
...
function BehaviorStarfall:Evaluate()
	...
	if #enemies > 0 then
		desire = 4
		if ABILITY_Starfall and ABILITY_Starfall:IsFullyCastable() then
			thisEntity:Stop()
...
function BehaviorStarfall:Begin()
	...
	self.order =
	{
		...
		AbilityIndex = ABILITY_Starfall:entindex(),
	}
...
AICore.possibleBehaviors = { BehaviorNone, BehaviorStarfall, BehaviorRunAway }
