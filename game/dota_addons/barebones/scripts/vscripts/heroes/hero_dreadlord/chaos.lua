...
function invoker_chaos_meteor_datadriven_on_spell_start(keys)
	...
	local chaos_meteor_fly_particle_effect = ParticleManager:CreateParticle("particles/units/heroes/hero_dreadlord/chaos_2_fly.vpcf", PATTACH_ABSORIGIN, keys.caster)
	...
	local exort_ability = keys.caster:FindAbilityByName("holdout_chaos")
	local main_damage = 0
	local burn_dps = 0
	...
	local wex_ability = keys.caster:FindAbilityByName("holdout_chaos")
